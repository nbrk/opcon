...
import Opcon.HierarchyNode
import Opcon.Command
...
-- | Attach a sub-hierarchy to the topmost hq of the
--   given hierarchy. The top-hq's echelon must be
--   superior to the attaching force.
attachHierarchy :: (Organization a, EchelonLevel e)
                => Hierarchy a e -> Bool
                -> Hierarchy a e -> Hierarchy a e
attachHierarchy subh organic hier =
  let hierhq = getHqNode hier
  in
    attachHierarchyToHq subh organic hierhq hier


-- | Attach a sub-hierarchy to the given node (hq) inside
--   of a hierarchy. The given hq-node's echelon must be
--   superior to the attaching force.
attachHierarchyToHq :: (Organization a, EchelonLevel e)
                => Hierarchy a e -> Bool ->HierarchyNode a
                -> Hierarchy a e -> Hierarchy a e
attachHierarchyToHq subh organic hqn hier =
